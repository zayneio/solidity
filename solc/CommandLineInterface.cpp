...
map<string, Json::Value> CommandLineInterface::parseAstFromInput()
{
	...
	for (auto const& sourceUnitID: m_fileReader.sourceUnitIDs())
	{
		Json::Value ast;
		astAssert(jsonParseStrict(m_fileReader.sourceCode(sourceUnitID), ast), "Input file could not be parsed to JSON");
...
bool CommandLineInterface::processInput()
{
	...
	if (m_args.count(g_argStandardJSON))
	{
		...
		StandardCompiler compiler([this](auto a, auto b) { return m_fileReader.readFile(a, b); });
	...
	m_compiler = make_unique<CompilerStack>([&](auto a, auto b) { return m_fileReader.readFile(a, b); });
