...
void CompilerStack::setRemappings(vector<FileRemapper::Remapping> const& _remappings)
{
	if (m_stackState >= ParsedAndImported)
		BOOST_THROW_EXCEPTION(CompilerError() << errinfo_comment("Must set remappings before parsing."));
	for (auto const& remapping: _remappings)
		solAssert(!remapping.prefix.empty(), "");
	m_fileRemapper.setRemappings(_remappings);
}
...
{
	solAssert(m_stackState < ParsedAndImported, "");
	return m_fileRemapper.apply(_path, _context);
}
...
string CompilerStack::createMetadata(Contract const& _contract) const
{
	...
	for (auto const& r: m_fileRemapper.remappings())
