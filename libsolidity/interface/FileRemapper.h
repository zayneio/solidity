...
using SourceUnitID = std::string;
using SourceCode = std::string;
using ImportPath = std::string;

/// The FileRemapper is being used on imported file paths for being remapped to source unit IDs before being loaded.
class FileRemapper
{
public:
	struct Remapping
	...
	SourceUnitID apply(ImportPath const& _path, std::string const& _context) const;
