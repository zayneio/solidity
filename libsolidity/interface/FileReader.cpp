...
void FileReader::setSource(SourceUnitID _sourceUnitID, SourceCode _source)
{
	m_sourceCodes[_sourceUnitID] = std::move(_source);
}

void FileReader::setSource(SourceUnitID _sourceUnitID, optional<boost::filesystem::path> _fspath, SourceCode _source)
{
	if (_fspath.has_value())
		m_sourceCodeOrigin[_sourceUnitID] = _fspath.value();
