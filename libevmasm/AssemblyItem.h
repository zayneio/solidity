...
public:
	...
	bytes const& verbatimData() const { assertThrow(m_type == VerbatimBytecode, util::Exception, ""); return m_verbatimData->second; }
	...
	size_t deposit() const { return m_type == VerbatimBytecode ? static_cast<size_t>(m_verbatimData->first) : returnValues() - arguments(); }
...
private:
	...
	std::optional<std::pair<int, bytes>> m_verbatimData; ///< Only valid for m_type == VerbatimBytecode
